...
class puzzlebox_synapse_protocol_thinkgear(threading.Thread):
	...
	def resetSessionStartTime(self):
		...
		if (self.parent != None):
			...
			#self.parent.updateSessionStartTime(self.session_start_timestamp)
			self.parent.resetSessionStartTime()
	...
	def resetSession(self):
		...
		self.parent.setBadPackets(0)
		#self.session_start_timestamp = time.time()
		#self.parent.setSessionTimestamp( time.time() )
		#self.parent.resetSessionTimestamp()
		self.parent.resetSessionStartTime()
...
##				                    timeout = timeout)
	...
	def stop(self):
		...
		try:
			...
			if self.DEBUG:
				print "ERROR: Protocol failed to call self.buffer_check_timer.stop() in stop():",
				print e
		...
		try:
			...
			if self.DEBUG:
				print "ERROR: Protocol failed to call self.read_buffer_check_timer.stop() in stop():",
				print e
