from brainsquared.settings import DEVICE_METADATA
...
def map_metric_name_to_num_channels(device_name):
  ...
  """
  metadata = [metadata for metadata in DEVICE_METADATA 
              if metadata['device_name'] == device_name]

  if len(metadata) > 0:
    metrics = metadata[0]['metrics']
  else:
    raise ValueError("Could not find device name '%s' "
                     "in metadata" % device_name)
...
def get_metrics_names(device_type):
  ...
  """
  metadata = [metadata for metadata in DEVICE_METADATA 
              if metadata['device_name'] == device_type]

  if len(metadata) > 0:
    metrics = metadata[0]['metrics']
  else:
    raise ValueError("Could not find device name '%s' "
                     "in metadata" % device_type)
